#include <stdio.h>
#include "rolety.h"
#include "strukturaPokoju.h"
int rolety(pokoj *zaczytany)
/*
##########################################################################################################################################
PRE:
Wskaznik do struktury z pokojem.
POST:
Zmiana bitu ktory informowal o stanie rolety oraz kod bledu lub informacja o sukcesie.
CEL:
Przelaczyc stan rolety wedlug zyczenia uzytkownika.
OPIS DZIALANIA:
Wyswietlamy interfejsy do obslugi zmian warunkowo czyli zaleznie od obecnego stanu potem dajemy uzytkownikowi opcje by zmienic stan 
rolety z pomoca switcha.
OBSLUGA BLEDOW:
Kod 10 - blad przemieszczania sie po programie.
##########################################################################################################################################
*/
{
    int zmiennaPrzelaczajaca;
    printf("###################################################################################################################\n");
    if((*zaczytany).rolety == 0)
    {
        printf("#                                 Rolety odsloniete czy chcesz je opuscic?                                        #\n");
        printf("#                                 1. Tak                                                                          #\n");
        printf("#                                 2. Nie                                                                          #\n");
        printf("###################################################################################################################\n");
        printf("Twoja opcja: ");
        scanf("%d", &zmiennaPrzelaczajaca);
        switch (zmiennaPrzelaczajaca)
        {
        case 1:
            (*zaczytany).rolety = 1;
            break;
        case 2:
            return 0;
        default:
            fprintf(stderr, "Blad opcji do przemieszczania sie po programie\n");
            return 10;
        }
    }
    else
    {
        printf("#                                 Rolety zasloniete czy chcesz je podniesc?                                       #\n");
        printf("#                                 1. Tak                                                                          #\n");
        printf("#                                 2. Nie                                                                          #\n");
        printf("###################################################################################################################\n");
        printf("Twoja opcja: ");
        scanf("%d", &zmiennaPrzelaczajaca);
        switch (zmiennaPrzelaczajaca)
        {
        case 1:
            (*zaczytany).rolety = 0;
            break;
        case 2:
            return 0;
        default:
            fprintf(stderr, "Blad opcji do przemieszczania sie po programie\n");
            return 10;
        }
    }
    return 0;
}